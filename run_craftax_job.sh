...
NOW=$(date "+%Y.%m.%d")
TIME=$(date "+%H%M")
OUT_DIR="/home/chuaraym/scratch/exp_sweep/${NOW}/${TIME}_craftax/${SLURM_JOB_ID}_${SEED}"
mkdir -p "$OUT_DIR"
...
echo "Running Craftax experiment with SEED=${SEED}"
...
source /home/chuaraym/pqn_env311/bin/activate

# Change to project root (so purejaxql is importable)
cd /home/chuaraym/purejaxql/

# Run the experiment
python purejaxql/pqn_sf_craftax.py +alg=pqn_sf_craftax SEED=${SEED} SAVE_PATH=${OUT_DIR} PROJECT="Craftax" alg.TOTAL_TIMESTEPS=1e10 alg.TOTAL_TIMESTEPS_DECAY=1e10