...
def make_train(config):
    ...
    def train(rng, exposure, train_state, network, task_id, unique_task_id):
        ...
        train_state = train_state.replace(exploration_updates=0, total_returns_per_task=0)
        ...
        def _update_step(runner_state, unused):
            ...
            def _step_env(carry, _):
                ...
                # if explore only first exposure, set eps to EPS_FINISH for all except the first exposure
                if config["EXPLORE_ONLY_FIRST_EXPOSURE"]:
                    if exposure == 0:
                        eps = jnp.full(
                            config["NUM_ENVS"],
            ...
            train_state = train_state.replace(
                ...
                total_returns_per_task=train_state.total_returns_per_task + transitions.reward.sum(),
            ...
            metrics = {
                ...
                * env.observation_space.shape[
                ...
                "td_loss": loss.mean(),
                "qvals": qvals.mean(),
                "eps": eps_scheduler(train_state.exploration_updates) if exposure == 0 else config["EPS_FINISH"],
                "lr": lr,
                "exposure": exposure,
                "task_id": task_id,
                ...
                "total_returns_per_task": train_state.total_returns_per_task,
            }
...
def single_run(config):
    ...
    env_names = "Centipede-v5, Alien-v5, Atlantis-v5, Boxing-v5, Breakout-v5"
    ...
    for cycle in range(num_exposures):
        ...
        for idx, env_name in enumerate(env_names):
            ...
            if unique_task_id == 0:
                ...
                config["TOTAL_TIMESTEPS_DECAY"] = 1000000
            else:
                config["TOTAL_TIMESTEPS"] = 900000
                config["TOTAL_TIMESTEPS_DECAY"] = 900000
