...
class SFNetwork(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray, task: jnp.ndarray, train: bool):
        ...
        if task.ndim == 1:
            ...
            task = jnp.repeat(task, x.shape[0], axis=0)
        ...
        new_sf = jnp.stack(new_sf, axis=1)  # (NUM_env, action_dim, sf_dim)

        # create q-values by multiplying the sf with the task
        q_vals = jnp.einsum("bna,ba->bn", new_sf, task)  # (NUM_env, num_actions)

        return q_vals, basis_features
...
def make_train(config):
    ...
    def train(rng):
        ...
        def init_meta(rng_key, sf_dim) -> chex.Array:
...
def single_run(config):
    ...
    alg_name = config.get("ALG_NAME", "pqn_sf")
