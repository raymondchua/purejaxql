...
class SFNetwork(nn.Module):
    ...
    @nn.compact
    def __call__(self, x: jnp.ndarray, task: jnp.ndarray, noise_rng: chex.Array, train: bool):
...
def make_train(config):
    ...
    def train(rng, exposure, train_state, network, task_id):
        ...
        def _update_step(runner_state, unused):
            ...
            (last_q, basis_features) = network.apply(
                {
                    "params": train_state.network_state.params,
                    "batch_stats": train_state.network_state.batch_stats,
                },
            ...
            def _learn_epoch(carry, _):
                ...
                return (train_state, rng), (loss, qvals, reward_loss, task_params_diff, entropy, probs)
...
def single_run(config):
    ...
    agent_train_state, network = create_agent(
            ...
            if config["NUM_SEEDS"] > 1:
                ...
                outs = jax.jit(
                    lambda rng: make_train(config)(
                        rng, cycle, agent_train_state, network, task_id
                    )
                )(rng)
