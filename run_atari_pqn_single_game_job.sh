...
NOW=$(date "+%Y.%m.%d")
TIME=$(date "+%H%M")
OUT_DIR="/home/chuaraym/scratch/exp_sweep/${NOW}/${TIME}_atari_single_game/${SLURM_JOB_ID}_${SEED}"
mkdir -p "$OUT_DIR"
...
# Run the experiment
python purejaxql/pqn_atari.py +alg=pqn_atari SEED=${SEED} SAVE_PATH=${OUT_DIR} PROJECT="Atari_Single_Game" alg.ENV_NAME="Alien-v5"